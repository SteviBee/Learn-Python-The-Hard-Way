...
class Engine(object):
    ...
    def __init__(self, scene_map):
        ...
        #print "scene_map variable from Engine class = ", scene_map
    ...
    def play(self):
        ...
        while True:
            ...
            # next_scene takes two param and returns the class w/e string is
            # returned. next_scene_name is how I call the fucnction "enter",
            # and each time next_scene_name is exectued it return a string
            # of the next scene therefore falling into the syntax of calling
            # a key/value pair. instatiates on the fly.
...
class Intro(object):
    ...
    def __init__(self):
        ...
        if x == 'yes':
            ...
            try:
                self.name = y
                # self.special = s[y] ## NOT NEEDED JUST FOR MY FYI
                # print "other way speical: ", s[y] ## NOT NEEDED JUST FOR MY FYI
            finally:
                s.close()
                e.close()
            ...
            e = shelve.open('enemey_shelf.db')
            try:
                # adding the code to make the instance of the Intro() class name
                # as the key with special and enemey to a dict as the value to recall later
                s[self.name] = self.special
                e[self.name] = self.enemey
            finally:
                #print "printing the special shelve: ", s
                #print "printing the enemey shelve: ", e
                s.close()
                e.close()
    ...
    def info(self):
        ...
        # this is a current dict of names to attributes but i can just use all shelf
        # self.specialities = {self.name:self.special}
        # self.enemeies = {self.name:self.enemey}

        #maybe a pickle function here to capture this data to a file?
...
# print "Should be special: ", intro.specialities[intro.name]
# print "other way special: ", intro.special ## NOT NEEDED JUST FOR MY FYI
# using shelve method to call things
s = shelve.open('speciality_shelf.db')
e = shelve.open('enemey_shelf.db')
try:
...
#     def enter(self):
#         print "Archery!"
#         x = raw_input("hit enter to go to sword fighting!")
#         return 'sword_fight'
...
class Map(object):
    ...
    def next_scene(self, scene_name):
        ...
        #print "Tranisitioning to next scene: ", scene_name
        # use get method to return value (class) from scenes dict
        return Map.scenes.get(scene_name)

    def opening_scene(self):
        # key here is the scene (string) passed with the start of the Map class
        # it returns the class of that scene.
